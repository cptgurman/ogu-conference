                        ...
                        <table class="table table-hover text-nowrap">
                            ...
                            <tbody>
                                @foreach ($event_types as $event_type)
                                    <tr>
                                        <td>{{ $event_type->id }}</td>
                                        <td colspan="2">{{ $event_type->name }}</td>
                                        <td>
                                            <a href="{{ route('admin.event_type.show', $event_type->id) }}"> <i
                                                    class="fa fa-eye"></i></a>
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.event_type.edit', $event_type->id) }}"> <i
                                                    class="fa fa-pen text-green"></i></a>
                                        </td>
                                        <td>
                                            <form action="{{ route('admin.event_type.delete', $event_type->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
